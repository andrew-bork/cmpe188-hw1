...
def get_task_metadata():
    ...
    return {
        ...
        "description": "Fit a neural network on Fashion MNIST using a neural network."
    }
...
def make_dataloaders(validation_split=0.2, batch_size=32, device=None):
    ...
    train_dataset = torchvision.datasets.FashionMNIST(".data/fashion_mnist", True, download=True, transform=torchvision.transforms.ToTensor())
    val_dataset = torchvision.datasets.FashionMNIST(".data/fashion_mnist", False, download=True, transform=torchvision.transforms.ToTensor())
...
def save_artifacts(model, train_losses, val_losses, output_dir="output", filename_prefix="nn_mnist"):
    ...
    plt.tight_layout()
    plt.savefig(f"{filename_prefix}_plot.png", dpi=150, bbox_inches='tight')
    plt.close()
