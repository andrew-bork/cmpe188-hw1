"""
Softclass Regression applied to Taiwan bankruptcy dataset
...
def get_task_metadata():
    ...
    return {
        "task_name": "logistic_bankruptcy",
        "task_type": "classification",
        "num_classes": 2,
        "input_dim": 95,
        "description": "Softmax Regression applied to Taiwan bankruptcy dataset"
    }
...
def make_dataloaders(batch_size=32, validation_split=0.8, device=None):
    ...
    csv = pd.read_csv('data/bankruptcy.csv')
    
    cols = csv.columns[1:]
    cols = [ c for c in cols if c != " Net Income Flag"]
    # print(cols)
    ...
    training_target, training_features = torch.LongTensor(training[csv.columns[0]].to_numpy()).to(device), torch.as_tensor(training[csv.columns[1:]].to_numpy()).to(device).to(torch.float32)
    validation_target, validation_features = torch.LongTensor(validation[csv.columns[0]].to_numpy()).to(device), torch.as_tensor(validation[csv.columns[1:]].to_numpy()).to(device).to(torch.float32)
...
def save_artifacts(model, metrics, X_train, y_train, X_val, y_val, 
                   output_dir="output", filename_prefix="logistic_bankruptcy"):
